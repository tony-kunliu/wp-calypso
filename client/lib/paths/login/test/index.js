...
describe( 'index', () => {
	describe( '#login()', () => {
		...
		it( 'should return the login url when the two factor auth page is supplied', () => {
			const url = login( { native: true, twoFactorAuthType: 'code' } );
		...
		it( 'should return the login url with encoded redirect url param', () => {
			const url = login( { native: true, redirectTo: 'https://wordpress.com/?search=test&foo=bar' } );
